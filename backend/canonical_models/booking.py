...
class BookingLineItem(BaseModel):
    ...
    title: str
    qty: int = Field(ge=1)
    # Client prices are advisory only; server recomputes from DB.
    unit_price_paise: Optional[int] = Field(default=None, ge=0)
    total_price_paise: Optional[int] = Field(default=None, ge=0)
    meta: dict[str, Any] = {}
...
class BookingIntentBase(BaseModel):
    ...
    # Advisory only. Server recomputes from DB pricing.
    total_amount_paise: Optional[int] = Field(default=None, ge=0)
